...
void main(List<String> args) {
  ...
  stdout.writeln('Nursery number + early-math simulator');
  stdout.writeln('Seeds per math skill: $seeds');
  stdout.writeln('Checks run: ${report.checksRun}');
  for (final finding in report.findings) {
    stdout.writeln(finding);
  }
  if (report.hasReleaseBlockingFindings) {
    stderr.writeln('FAIL: number/math correctness findings detected.');
    exitCode = 1;
  } else {
    stdout.writeln('PASS: number/math simulations are release-blocker free.');
  }
}
